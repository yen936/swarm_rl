...
async function init() {
    ...
    try {
        console.log('Fetching replay.json...');
        ...
        console.log('Replay data loaded:', replayData);

        if (!replayData.frames || !Array.isArray(replayData.frames) || replayData.frames.length === 0) {
            throw new Error('Invalid replay data format: missing or empty frames array');
        }

        simulationData = {
            world: {
                size_x: replayData.metadata.world_size.x,
                size_z: replayData.metadata.world_size.z
            },
            buildings: replayData.buildings || [],
            drones: {},
            frames: []
        };

        // Initialize drones from first frame
        const firstFrame = replayData.frames[0];
        if (!firstFrame.red_drones || !firstFrame.blue_drones) {
            throw new Error('Invalid frame format: missing drone data');
        }

        console.log('Processing first frame:', firstFrame);

        firstFrame.red_drones.forEach((drone, index) => {
            const id = `red_${index + 1}`;
            simulationData.drones[id] = { team: 'red' };
        });
        firstFrame.blue_drones.forEach((drone, index) => {
            const id = `blue_${index + 1}`;
            simulationData.drones[id] = { team: 'blue' };
        });

        // Convert each frame
        replayData.frames.forEach(frame => {
            const convertedFrame = {
                drones: {}
            };

            // Convert red drones
            frame.red_drones.forEach((drone, index) => {
                const id = `red_${index + 1}`;
                convertedFrame.drones[id] = {
                    x: drone.x,
                    y: drone.y,
                    z: drone.z
                };
            ...
            // Convert blue drones
            frame.blue_drones.forEach((drone, index) => {
                const id = `blue_${index + 1}`;
                convertedFrame.drones[id] = {
                    x: drone.x,
                    y: drone.y,
            ...
            simulationData.frames.push(convertedFrame);
        });

        console.log('Loaded and converted simulation data:', simulationData);
    } catch (error) {
